...
import { getProvider, getTokenMessengerMinterProgram } from "./utils";
...
export const DEFAULT_MIN_FINALITY_THRESHOLD = 1000;

export const depositForBurnSol = async (
  amount: BN,
  maxFee: BN,
  minFinalityThreshold: number,
  destinationCaller: string = process.env.REMOTE_EVM_ADDRESS
): Promise<string> => {
  ...
  return await tokenMessengerMinterProgram.methods
    .depositForBurn({
      ...
      destinationCaller: new PublicKey(getBytes(evmAddressToBytes32(destinationCaller))),
    })
    .accounts({
